...
{
	/**
	 * The string representation of the number with a grouping separator every 3 digits.
	 */
	...
	{
		requireThat(name, "name").doesNotContainWhitespace().isNotEmpty();
		properties.add(new SimpleImmutableEntry<>(name, FORMATTER.get().format(number)));
		return this;
	}
